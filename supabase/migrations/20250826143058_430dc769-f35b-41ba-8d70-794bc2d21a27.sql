-- Fix gen_salt function calls to use correct schema
...
BEGIN
  ...
  -- Create auth user with temporary password - use extensions schema for gen_salt
  INSERT INTO auth.users (
    ...
    p_email,
    crypt(temp_password, extensions.gen_salt('bf')),
    now(),
...
BEGIN
  ...
  -- Update auth user password - use extensions schema for gen_salt
  UPDATE auth.users 
  SET encrypted_password = crypt(temp_password, extensions.gen_salt('bf'))
  WHERE id = p_user_id;
